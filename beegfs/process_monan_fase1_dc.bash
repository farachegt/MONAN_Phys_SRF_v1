...
rm -f latlon.nc
var2d=u_gustfront,v_gustfront,uzonal_850hPa,umeridional_850hPa,wlpool,vcpool,v2d1,rmfxshcu,rtopdpcu,\
...
var2d_aval=gsw,lh,hfx,t2m,rmfxdpcu,rmfxshcu,cldfrac_tot_UPP,rainnc,rainc,rmfxmdcu,\
...
if [ ${doconvert} -ge 1 ] ; then 
  ...
  do
     ...
     file=$x
     if [[ $x -lt 10 ]] ; then file=$zero$x ; fi

     echo 'diag/hist file=' $file ${f}

     if [[ -e ${f} ]] && [[ ${doconvert} == 1 ]]; then 

         ./convert_mpas file_grid_spc.init.nc ${f} &>> dc.out
         /bin/mv latlon.nc ${file}_ll_${f}
         /bin/mv ${f} backup

         #-- get the hourly precip
         if [[ $x -eq 0 ]] ; then 
            #-- echo 'copying ' ${file}_ll_${f} previous_precip.nc
            /bin/cp ${file}_ll_${f} previous_precip.nc
         fi
         #echo 'precip between: '$x ${file}_ll_${f} previous_precip.nc
         cdo -L sub -apply,-selname,rainc,rainnc [ ${file}_ll_${f} previous_precip.nc ] ${file}_ll_precip_${f}

         #-- save for next step
         /bin/cp ${file}_ll_${f} previous_precip.nc

         #-- for troposphere avaliation
         cdo selname,${var2d_aval} ${file}_ll_${f} ${file}_2d_aval_ll_$f

         #-- put all files into only one  ${f1} and ${f2} 
         cdo cat ${file}_ll_${f}        ${f1}
         cdo cat ${file}_ll_precip_${f} ${f2}
         cdo cat ${file}_2d_aval_ll_$f  ${f3}

         #-- move processed files to backup2
         /bin/mv  ${file}_ll_${f} ${file}_ll_precip_${f}  ${file}_2d_aval_ll_$f backup2
         ...
         #cdo -L -sellevel,1/42 -selname,${var3d} ll_$f 3d_ext_ll_$f
